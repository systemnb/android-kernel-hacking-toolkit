/*
 * Android property modification driver - write via physical address
 * Suitable for Android 14+ (kernel 6.1)
...
struct prop_write_args {
    uint64_t value_addr;   // user space address
    char new_value[PROP_VALUE_MAX];
};

/* Page table walk: translate virtual address to physical address */
...
/* Write to physical address (single page only) */
static int write_physical_address(phys_addr_t pa, const char *buf, size_t len)
{
    ...
    /* Ensure we do not cross a page boundary (property value max 92, safe) */
