/*
 * syscall_hijack_sysinfo.c - Hijack sysinfo system call (aarch64)
 * Through ioctl, set fake system info; all processes calling sysinfo() will see the fake data.
 * Uses mmuhack to disable write protection of sys_call_table.
 *
 * Compile: make -C /lib/modules/$(uname -r)/build M=$PWD modules
 * Load: insmod syscall_hijack_sysinfo.ko
 * Test: Use userspace program to set fake info (e.g., ./set_fake_sysinfo)
 * Observe effect: run free -m or write a small program that reads sysinfo.
 */
...
/* Generic kallsyms lookup function (compatible with 5.7+) */
...
/* Custom sysinfo: tamper with returned data */
...
static long hijack_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case SET_FAKE_SYSINFO:
        ...
        /* Enable fake data as long as the structure is not all-zero, or user can set a flag */
        fake_enabled = 1;
